  ...
  Object.entries(props.payload)
    .map(([key, value]) => {
      ...
      const indicatorColor =
        props.config[key]?.color ?? (props.payload as { fill?: string }).fill;
...
const tooltipLabel = computed<string | number | Date | null | undefined>(() => {
  ...
  return props.labelKey
    ? (props.config[props.labelKey]?.label as string | undefined) ??
        (props.payload as Record<string, string | number>)[props.labelKey]
    : props.x;
});
</script>
