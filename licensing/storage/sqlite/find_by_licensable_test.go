...
func TestSqlite_FindByLicensable_AcrossScopes(t *testing.T) {
	s := freshSqlite(t)
...
func TestSqlite_FindByLicensable_ScopedFilter(t *testing.T) {
	s := freshSqlite(t)
	...
	if globalOnly[0].ScopeID != nil {
		t.Fatalf("expected nil scope on global match")
	}
}

func TestSqlite_FindByLicensable_NotFound(t *testing.T) {
	s := freshSqlite(t)
	matches, err := s.FindLicensesByLicensable(lic.FindByLicensableQuery{
		Type: "User", ID: "u_nope",
	})
